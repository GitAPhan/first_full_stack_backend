...
def patch_candy():
    ...
    try:
        ...
        return Response("Input Error: incorrect keyvalue entered for 'id'", mimetype="plain/text", status=400)
    except:
        return Response("Generic patch request error", mimetype="plain/text", status=400)

    # request from database
    patch_status_message, patch_status_code = db.patch_candy_db(id, name, description)

    return Response(patch_status_message, mimetype="plain/text", status=patch_status_code)
...
def delete_candy():
    ...
    return Response(delete_status_message, mimetype="plain/text", status=delete_status_code)
